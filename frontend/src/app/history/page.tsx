  ...
  const fetchData = async (nextPage: number) => {
    ...
    try {
      ...
      if (!summaryRes.ok || !historyRes.ok) {
        const detail = await historyRes.text();
      ...
      setError("백테스트 데이터를 불러오지 못했습니다. DB 설정과 backfill 실행 여부를 확인하세요.");
    }
  };
  ...
  return (
    <main className="max-w-6xl mx-auto p-4 md:p-6 pb-20 space-y-4">
      ...
      <section className="bg-color-card border border-color-card-border rounded p-4">
        <div className="flex flex-wrap gap-2 items-end">
          <label className="text-xs flex flex-col gap-1">
            시작일
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-[#0f172a] border border-[#334155] rounded px-2 py-1" />
          </label>
          <label className="text-xs flex flex-col gap-1">
            종료일
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-[#0f172a] border border-[#334155] rounded px-2 py-1" />
          </label>
          <label className="text-xs flex flex-col gap-1">
            수수료(bps)
            <input type="number" min={0} value={feeBps} onChange={(e) => setFeeBps(Number(e.target.value))} className="bg-[#0f172a] border border-[#334155] rounded px-2 py-1 w-24" />
          </label>
          <label className="text-xs flex flex-col gap-1">
            슬리피지(bps)
            <input type="number" min={0} value={slippageBps} onChange={(e) => setSlippageBps(Number(e.target.value))} className="bg-[#0f172a] border border-[#334155] rounded px-2 py-1 w-24" />
          </label>
          <button className="px-3 py-1.5 text-sm rounded bg-[#1e40af] hover:bg-[#1d4ed8]" onClick={() => fetchData(1)}>
            조회
          </button>
        </div>
      </section>
      ...
      <section className="border border-color-card-border rounded overflow-hidden">
        <div className="grid grid-cols-5 gap-2 px-3 py-2 text-xs bg-[#1c2128] text-color-muted">
          <span>날짜</span>
          <span>종목</span>
        ...
        {items.map((item) => (
          <div key={`${item.tradeDate}-${item.ticker}`} className="grid grid-cols-5 gap-2 px-3 py-2 text-sm border-t border-[#2d333b] bg-[#0f172a]">
      ...
      <div className="flex items-center justify-end gap-2">
        <button disabled={page <= 1} onClick={() => fetchData(page - 1)} className="px-2 py-1 rounded bg-[#1e293b] disabled:opacity-40">
          이전
        </button>
        <span className="text-sm text-color-muted">
          {page} / {maxPage}
        </span>
        <button disabled={page >= maxPage} onClick={() => fetchData(page + 1)} className="px-2 py-1 rounded bg-[#1e293b] disabled:opacity-40">
          다음
        </button>
      </div>
    </main>
